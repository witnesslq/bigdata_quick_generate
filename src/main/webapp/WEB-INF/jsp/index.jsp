...
<script type="text/javascript">
    ...
    //---------修改人NumberOne完毕----------
		...
		$("[nav-n]").each(function (){
			if($(this).attr("nav-n").indexOf('表信息')!=-1){
				$(this).trigger("click");
			}
		});
		});
											...
											<li <c:if test="${s.index==0}">class="active"</c:if>><a
												...
												<ul class="nav lt">
													<c:forEach var="kc" items="${key.children}">
														<li class="active"><a
															href="javascript:void(0)"
															class="active" nav-n="${key.resNm},${kc.resNm},${kc.resurl}?id=${kc.resId}"> <i class="fa fa-angle-right"></i> <span>${kc.resNm}</span>
														</a></li>
													</c:forEach>
												</ul></li>
										</c:forEach>
