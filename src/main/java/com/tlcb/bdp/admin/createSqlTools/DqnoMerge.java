...
/*		Set<ExcelObject> codeFieldSet = null;//码值
		...
		sb.append("select COUNT(*) into val_COUNT_DB2 from ").append(tableName).append("@db2_edwinst_").append(schema.trim().toLowerCase()).append(" WHERE cast(last_etl_acg_dt as string) = v_acctdt and del_f!=2;");sb.append("\r\n");
		
		for(ExcelObject amount:amountFieldSet){
			if(amount.getDB_NM().equals(schema.trim()) && amount.getTAB_NM().equals(tableName)){
				sb.append("select SUM(abs(").append(amount.getTAB_FLD_NM()).append(")) into val_").append(amount.getTAB_FLD_NM()).append("_DB2 from ").append(tableName).append("@db2_edwinst_").append(schema.trim().toLowerCase()).append(" WHERE cast(last_etl_acg_dt as string) = v_acctdt and del_f!=2;");sb.append("\r\n");
			}
		}
		sb.append("\r\n");
