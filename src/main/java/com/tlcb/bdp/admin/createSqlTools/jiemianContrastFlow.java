...
public class jiemianContrastFlow {
	/****
	 * 
	 * 截面表与拉链表中取出截面对比存储过程---流水型
	 * @param tableName
	 * @param columnList
	 * @return
	 */
	public static String  generateIncPro(String schema,String tableName,List<ColumnMeta> columnList) {
		...
		sb.deleteCharAt(sb.lastIndexOf(","));
		sb.append(" from "+schema.trim()+"."+tableName+"_HS where LAST_ETL_ACG_DT=v_acctdt));");sb.append("\r\n");
		...
		sb.deleteCharAt(sb.lastIndexOf(","));
		sb.append(" from ").append(schema.trim()+"."+tableName+"_HS where LAST_ETL_ACG_DT=v_acctdt)");
		...
		sb.append("v_chk_flag := 'OK';");sb.append("\r\n");
		...
		sb.append("insert into table DQ.PROC_LOG (proc_name,proc_act_dt,proc_stepnum,proc_stepdesc,proc_sqlcode,proc_sqlerrm,proc_step_flg,cur_ts) values "
				+ "(v_proc_name,v_acctdt,v_proc_stepnum,v_proc_stepdesc,v_sqlcode,v_sqlerrm,v_proc_step_flg,systimestamp);");sb.append("\r\n");
